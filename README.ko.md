...
> **기준일: 2026-08-21.** 이 문서는 제품 불변식과 목표 아키텍처의 기준 문서입니다. 구현 완료 여부는 `PROGRESS.md`, 현재 코드·테스트 결과, `docs/CURRENT_IMPLEMENTATION_AND_BACKLOG.md`를 함께 확인합니다. 목표 기능을 현재 구현으로 오해하지 않도록, 이 문서에서는 **현재 구현**, **출시 전 보완**, **장기 목표**를 구분합니다.

| 구분 | 현재 기준 |
| :--- | :--- |
| 현재 구현 | React 19·TypeScript·Vite와 Spring Boot 4.1·Java 25, Supabase PostgreSQL/Auth/Storage 기반의 웹 MVP입니다. 판매자 신청·증빙, 카탈로그·장바구니·주문, 매칭·슬롯·패널티, 판매점 영업 상태·별도 찾아오시는 길, 구독, 개발 결제 승인·정산·환불, 클레임·쿠폰·관리자 운영이 DB·REST API로 연결돼 있습니다. |
| 권한·인증 | `users.role`·`admin_level`·`user_feature_permissions`가 역할과 일반관리자 세부 기능을 판정합니다. 최고관리자는 일반관리자 부여·해지와 기능 토글을 관리하며, 변경 이력은 권한·관리자 역할 감사 테이블에 보관합니다. |
| 현재 검증 경계 | Flyway V1~V39, 서버 도메인 테스트, 프론트엔드 린트·타입 검사·프로덕션 빌드가 기준입니다. 실제 PG, NTS 진위확인, 채널별 푸시·이메일·SMS, Realtime/푸시, Redis 분산 운영, 법정 PDF 서류는 출시 전 보완 항목입니다. |
| 제품 규칙 우선순위 | 주문 상태, 단일 낙찰, 가용 슬롯 회계, 결제 순서, 클레임 정산 보류는 구현 방식이 바뀌어도 훼손하지 않습니다. |
...
| **지역 자격 조건** | 매칭은 배달 또는 픽업 지역의 `gu_code`를 기준으로 제한합니다. | 현재는 서울특별시·구 단위 배송지 검증과 DB 지역 코드로 운영합니다. 카카오 로컬 API 주소 정규화는 출시 전 외부 연동으로 보완합니다. |
...
| **판매자 멤버십 제안** | 비구독 판매자는 실버 기본 등급이며, 프리미엄·골드·실버 순으로 기회를 제공합니다. | 프리미엄은 주문 후 0~30초 독점 제안, 30~60초에는 프리미엄·골드 경쟁, 60초 이후에는 전체 판매자가 경쟁합니다. 동일 단계의 낙찰자는 서버가 단일로 판정합니다. |
| **클레임 보호** | 반품, 교환, 부분 교체 요청은 관련 정산 대금을 보류합니다. | 클레임 생성 트랜잭션에서 정산을 HOLD로 전환하고, 변경 불가능한 클레임 타임라인을 남깁니다. |
...
| **장바구니** | 수정 가능한 품목, 소계, 배달/픽업 선택, 배송지, 쿠폰 선택, 가격대·브랜드 안내 동의, 매칭 요청 CTA. | 주소·수량·필수 구매자 정보와 필요한 동의가 유효하기 전에는 주문 요청을 차단합니다. |
...
| **주문 상세 / 클레임** | 이행 타임라인, 결제·환불 이력, 클레임 CTA, 증빙 업로드, 클레임 이력. | 클레임 접수 시 정산 보류 효과를 설명하고, 서버가 확정한 상태만 표시합니다. 거래 서류 PDF는 출시 전 보완 항목입니다. |
...
| **빠른 슬롯 제어** | 가용 수량 슬라이더 또는 스테퍼. 등급별 한도는 프리미엄 1~15, 골드 1~8, 실버(비구독 포함) 1~3입니다. | 모든 변경은 서버 슬롯 이력으로 보관하며, UI는 대기·오류·완료 상태를 보여줍니다. |
...
현재 client/src/App.tsx는 ThemeProvider, IdentityProvider, AuthProvider, RouterProvider, 공통 알림과 전 화면 맨 위로 이동 버튼을 조합하는 앱 셸입니다. 제품 UI는 역할별 라우트와 기능 모듈로 분리되어 있습니다.

```text
client/
└── src/
    ├── app/                 # 앱 셸, 라우팅, 프로바이더, 라우트 가드
    ├── api/                 # Axios 단일 채널 REST API 계약
    ├── components/          # 재사용 UI 프리미티브와 공통 컴포넌트
    ├── features/
    │   ├── admin/           # 주문·판매자·정산·쿠폰·행사 운영
    │   ├── auth/            # 로그인·회원가입
    │   ├── cart/            # 장바구니와 장바구니 품목 상태
    │   ├── catalog/         # 카테고리, 검색, 상품 상세
    │   ├── checkout/        # 배송지, 주문 요청, 개발 결제 흐름
    │   ├── events/          # 행사 페이지
    │   ├── matching/        # 매칭 상태와 마감 시간 UI
    │   ├── my/              # 회원정보·배송지·결제수단
    │   ├── orders/          # 소비자 주문 추적·클레임
    │   ├── seller/          # 가용량, 주문 제안, 확인, 이행, 구독
    │   ├── stores/          # 판매점 목록·상세·후기
    │   └── support/         # 고객 문의·안내
...
라우트 가드는 CONSUMER, SELLER, ADMIN 역할을 강제합니다. 비즈니스 판단은 서버에 두고, 프론트엔드는 확정된 상태를 렌더링하고 유효한 입력을 수집합니다. 현재 갱신은 REST API와 필요한 폴링을 사용하며, Supabase Realtime·푸시 구독은 출시 전 확장 항목입니다.
...
flowchart LR
    ...
    API --> STORE[Supabase Storage: seller documents and claim evidence]
    SCH[Spring Scheduler] --> DB
    API -. 출시 전 확장 .-> REDIS[Redis: distributed locks and delayed offers]
    API -. 출시 전 확장 .-> RT[Supabase Realtime / Push Adapter]
    API -. 출시 전 외부 연동 .-> KAKAO[Kakao Local API]
    API -. 출시 전 외부 연동 .-> TOSS[Toss Payments and verified webhooks]
```

| 레이어 | 책임 |
| :--- | :--- |
| **React + TypeScript 클라이언트** | 역할별 UI, 반응형 레이아웃, 장바구니 상태, Axios 기반 REST API 렌더링을 현재 구현합니다. Realtime 구독은 출시 전 확장입니다. |
| **Spring Boot 4.1 API / Java 25** | 권한 검증, 도메인 상태 전이, 매칭 오케스트레이션, 개발 결제 승인·클레임·정산·쿠폰 워크플로우, 감사 로그를 현재 구현합니다. |
| **PostgreSQL / Supabase** | 영속 트랜잭션 상태, Auth, Storage, Flyway V1~V39를 현재 사용합니다. V34로 public 테이블 RLS 활성화와 직접 권한 회수를 적용했으며, 배포 환경에서는 정책·Storage·서비스 키 경계를 회귀 점검합니다. |
| **Redis** | 다중 인스턴스용 분산 락, 지연 제안 발송, 슬롯 예약, 재시도 조정의 출시 전 확장입니다. 현재 단일 낙찰 무결성은 PostgreSQL 잠금과 제약으로 방어합니다. |
| **Supabase Realtime / 푸시 어댑터** | 주문·클레임·환불 이벤트 전달을 위한 출시 전 확장입니다. 현재 화면은 REST API 갱신과 알림 저장을 사용합니다. |
...
| **Payment** | READY → PAID → CANCELLED 또는 PAID → PARTIAL_REFUNDED / REFUNDED | 현재는 개발 승인과 개발 환불 상태를 구현했습니다. 실제 PG 승인·서명 웹훅·거래 재조회는 출시 전 외부 연동입니다. |
| **Claim** | REQUESTED → SELLER_REVIEWING → PICKUP_SCHEDULED / REPLACEMENT_SHIPPING / ESCALATED → RESOLVED 또는 REJECTED | REJECTED는 사유와 감사 기록이 필수이며, 클레임 생성 시 정산을 HOLD로 변경합니다. |
| **Settlement** | PENDING → HOLD → RELEASABLE → SETTLED 또는 CANCELLED | 클레임·환불과 정산 원장은 현재 구현돼 있으며, 일일 자동 정산 가능 전환은 출시 전 보완합니다. |
...
낙찰 응찰, 슬롯 예약·해제, 클레임 정산 보류, 개발 결제 기록처럼 즉시 사용자가 수행하는 동작은 도메인 트랜잭션에서 처리합니다. 스케줄러는 마감 집행과 복구를 위한 안전망이며, 서버 시각을 사용합니다.

| 작업 | 현재 상태 | 주기·동작 | 출시 전 보완 |
| :--- | :--- | :--- | :--- |
| 주문 제안 확산 | 구현 | 1초 주기로 프리미엄 0~30초, 프리미엄·골드 30~60초, 전체 판매자 60초 이후 단계를 평가합니다. | Redis 지연 큐·다중 인스턴스 조정 |
| 주문 제안 만료 | 구현 | 10초 주기로 만료된 제안을 처리하고 예약 슬롯을 해제합니다. | 구조화된 지표·재시도 로그 |
| 최초 매칭 마감 | 구현 | 30초 주기로 5분 마감 주문을 처리합니다. | 예약 주문·재입찰 알림 정책 고도화 |
| 판매자 물품 확인 마감·패널티 | 구현 | 10초 주기로 2분 확인 기한을 집행하고 이력·제한을 반영합니다. | 누락 사고 복구 점검·지표 강화 |
| 구독 만료 | 구현 | 60초 주기로 만료 판매점을 실버 기본 등급으로 정규화하고 이력을 저장합니다. | 운영 알림·일괄 관리 정책 |
| PG 정합성·일일 정산·쿠폰 만료·신뢰 점수·PDF 재시도 | 출시 전 보완 | 실제 PG·문서·운영 데이터가 연결된 뒤 도입합니다. | 멱등 키, 외부 조회, 감사 로그, 재시도 정책을 포함한 구현 |
...
| 단계 | 현재 상태 | 완료·보완 기준 |
| :--- | :--- | :--- |
| 0. 제품 기반 | 구현 | 제품 규칙, 역할 라우트, 디자인 토큰, REST API 규약, Flyway 기반 스키마를 유지합니다. 티어 시간 규칙은 본 README의 0~30·30~60·60초 이후 기준을 사용합니다. |
| 1. 인증과 카탈로그 | 구현·운영 보완 | 인증, 카테고리·상품 CRUD, 판매자 신청·증빙 업로드·관리자 수동 심사가 구현돼 있습니다. NTS 실시간 진위확인과 배포 환경 RLS 점검이 남았습니다. |
| 2. 소비자 커머스 | 구현·외부 연동 보완 | 카탈로그, 장바구니, 배송지, 주문 요청, 가격대 동의가 구현돼 있습니다. Kakao 주소 정규화가 남았습니다. |
| 3. 매칭과 판매자 운영 | 구현·확장 보완 | 단일 낙찰, 가용 슬롯 회계, 물품 확인 마감, 패널티, 구독 티어, 판매자·관리자 운영이 구현돼 있습니다. Redis 기반 다중 인스턴스·대량 주문 확장이 남았습니다. |
| 4. 결제와 서류 | 부분 구현 | 개발 결제 승인, 개발 취소·부분/전액 환불, 정산 원장은 구현됐습니다. 실제 PG 승인·웹훅·재조회와 결정적 PDF 거래 서류가 남았습니다. |
| 5. 클레임·정산·거버넌스 | 구현·자동화 보완 | 클레임 증빙·상태 이력·정산 HOLD·판매자 처리·관리자 환불·쿠폰 운영이 구현됐습니다. 일일 정산, 사고 자동 구제, 채널별 실시간 알림을 보완합니다. |
| 6. 품질과 출시 | 진행 전 | 외부 서비스 운영 설정, 관측성, E2E·부하 시험, 접근성·모바일 QA, 백업·롤백·배포 절차를 완료합니다. |
...
| **백엔드** | Java 25 기반 Spring Boot 4.1, Spring Security, JPA, 명시적 도메인 서비스와 트랜잭션을 현재 사용합니다. |
| **데이터·실시간** | Supabase PostgreSQL, Auth, Storage를 현재 사용합니다. Realtime과 Redis 기반 락·가용량 조정·지연 처리는 출시 전 확장입니다. |
| **외부 서비스** | Kakao 주소 정규화, Toss 결제·환불 웹훅, NTS 사업자 진위확인은 출시 전 백엔드 어댑터와 운영 키 설정이 필요한 외부 연동입니다. |
...
| **테스트** | 현재 동시 응찰, 가용 슬롯, 타이머 마감, 개발 결제·환불, 클레임 HOLD, 쿠폰, 판매자 신청·증빙, 구독, 역할 권한을 도메인 테스트로 검증합니다. 실제 PG 웹훅·외부 API·다중 인스턴스 부하·E2E 시험은 출시 전 추가합니다. |
| **보안** | 서버 API의 역할·세부 기능 권한 검증, V34 RLS와 Storage 접근 경계, 환경 기반 비밀 관리, 파일 크기/유형/내용 검증을 현재 적용합니다. 실제 PG 웹훅 서명 검증과 운영 RLS·Storage 회귀 점검은 출시 전 완료합니다. |
| **감사 가능성** | 주문, 결제, 클레임, 슬롯 변경, 관리자 액션 타임라인을 보존합니다. 재무·거버넌스 상태는 행위자·시간·사유·상관관계 ID 없이 변경할 수 없습니다. |
| **관측성** | 매칭 지연, 주문 제안 만료, 슬롯 포화, 응찰 성공, 물품 확인 시간 초과, 결제 정합성, 클레임 처리 시간, 작업 실패를 출시 전 운영 지표와 경보로 연결합니다. 현재는 도메인 이력과 관리자 타임라인을 우선 사용합니다. |
