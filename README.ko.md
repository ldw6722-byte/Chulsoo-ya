...
> **기준일: 2026-08-18.** 이 문서는 제품 불변식과 목표 아키텍처의 기준 문서입니다. 구현 완료 여부는 PROGRESS.md와 현재 코드·테스트 결과를 함께 확인합니다. 목표 기능을 현재 구현으로 오해하지 않도록, 이 문서에서는 **현재 구현**, **출시 전 보완**, **장기 목표**를 구분합니다.

| 구분 | 현재 기준 |
| :--- | :--- |
| 현재 구현 | React 19·TypeScript·Vite와 Spring Boot 4.1·Java 25, Supabase PostgreSQL/Auth/Storage 기반의 웹 MVP입니다. 판매자 신청·증빙, 카탈로그·장바구니·주문, 매칭·슬롯·패널티, 구독, 개발 결제 승인·정산·환불, 클레임·쿠폰·관리자 운영이 DB·REST API로 연결돼 있습니다. |
| 현재 검증 경계 | Flyway V1~V31, 서버 도메인 테스트, 프론트엔드 린트·타입 검사·프로덕션 빌드가 기준입니다. 실제 PG, Kakao 주소, NTS 진위확인, Realtime/푸시, Redis 분산 운영, 법정 PDF 서류는 출시 전 보완 항목입니다. |
...
| **PostgreSQL / Supabase** | 영속 트랜잭션 상태, Auth, Storage, Flyway V1~V31을 현재 사용합니다. 배포 환경 RLS 점검은 출시 전 완료해야 합니다. |
