...
export function ShopHeader() {
  ...
  useEffect(() => { const keyword = query.trim(); if (!keyword) { setSuggestions([]); return } let active = true; const timer = window.setTimeout(() => { void catalogApi.suggestions(keyword).then(items => { if (active) setSuggestions(items.slice(0, 8)) }).catch(() => { if (active) setSuggestions([]) }) }, 150); return () => { active = false; window.clearTimeout(timer) } }, [query])
  function search(value = query) { const keyword = value.trim(); setSuggestions([]); if (keyword) navigate("/catalog?keyword=" + encodeURIComponent(keyword)) }
  ...
  return <header className="sticky top-0 z-50 border-b border-slate-200 bg-white/95 shadow-sm backdrop-blur dark:border-slate-700 dark:bg-slate-950">
    ...
    <div className="mx-auto flex max-w-7xl items-center gap-4 px-4 py-5">
      ...
      <div className="relative min-w-0 flex-1"><div className="flex overflow-hidden rounded-xl border-2 border-brand-500 bg-white dark:bg-slate-900"><input value={query} onChange={(event) => setQuery(event.target.value)} onBlur={() => window.setTimeout(() => setSuggestions([]), 120)} onKeyDown={(event) => { if (event.key === "Enter") search() }} placeholder={"\uC0C1\uD488\uBA85, \uADDC\uACA9, \uC6A9\uB3C4\uB85C \uAC80\uC0C9"} className="h-14 min-w-0 flex-1 bg-white px-4 text-sm text-slate-900 outline-none dark:bg-slate-900 dark:text-slate-100" aria-label={"\uC0C1\uD488 \uAC80\uC0C9"} /><button type="button" onClick={() => search()} className="grid w-14 place-items-center bg-brand-600 text-white" aria-label={"\uAC80\uC0C9"}><span className="h-5 w-5"><SearchIcon /></span></button></div>{suggestions.length ? <div className="absolute left-0 right-0 top-full z-50 mt-2 overflow-hidden rounded-xl border border-slate-200 bg-white py-1 shadow-xl dark:border-slate-700 dark:bg-slate-900">{suggestions.map((suggestion) => <button key={suggestion} type="button" onMouseDown={(event) => event.preventDefault()} onClick={() => { setQuery(suggestion); search(suggestion) }} className="flex w-full items-center gap-2 px-4 py-3 text-left text-sm font-medium text-slate-800 transition hover:bg-brand-50 hover:text-brand-700 dark:text-slate-100 dark:hover:bg-slate-800"><span className="text-brand-600">⌕</span><span className="truncate">{suggestion}</span></button>)}</div> : null}</div>
      <ThemeToggle compact />
