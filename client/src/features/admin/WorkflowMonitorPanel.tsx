...
const statusLabel: Record<string, string> = {
  WAITING_MATCH: '응찰 대기', SELLER_CONFIRMING: '판매자 확인', PAYMENT_PENDING: '결제 대기', PAID: '결제 완료',
...
function OrderTimeline({ order }: { order: AdminWorkflowOrder }) {
  const currentType: Record<string, string> = {
    WAITING_MATCH: 'ORDER_CREATED', SELLER_CONFIRMING: 'BID_WON', PAYMENT_PENDING: 'SELLER_CONFIRMED',
    PAID: 'PAYMENT', PREPARING: 'PAYMENT', DELIVERY_IN_PROGRESS: 'DELIVERY_STARTED', COMPLETED: 'COMPLETED',
  }
  return <ol className="mt-3 grid gap-2 border-l-2 border-slate-200 pl-4">{order.timeline.map(event => {
    const active = order.status !== 'COMPLETED' && currentType[order.status] === event.type
    const completed = event.type === 'COMPLETED'
    const dotClass = completed ? 'bg-emerald-500' : active ? 'bg-rose-500 animate-[pulse_2.4s_ease-in-out_infinite]' : 'bg-brand-500'
    return <li key={`${event.type}-${event.occurredAt}`} className="relative text-xs"><span className={`absolute -left-[1.42rem] top-1 h-2.5 w-2.5 rounded-full ring-4 ring-white ${dotClass}`} /><p className="font-black text-slate-800">{event.label}</p><p className="mt-0.5 text-slate-500">{dateTime(event.occurredAt)} {event.detail ? `· ${event.detail}` : ''}</p></li>
  })}</ol>
}
...
export function WorkflowMonitorPanel() {
  const orders = useAsync<AdminWorkflowOrder[]>(() => adminWorkflowApi.orders(), [], { pollMs: 2000 })
  ...
  return <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_22rem]">
    <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 px-5 py-4"><div><p className="text-xs font-black tracking-wider text-brand-600">ORDER LIFECYCLE</p><h2 className="mt-1 text-lg font-black text-slate-900">주문 · 응찰 모니터링</h2><p className="mt-1 text-sm text-slate-500">구매자 요청부터 응찰·낙찰·결제·배달 완료까지 서버 상태를 실시간으로 추적합니다. 빨간 점은 진행 중, 녹색 점은 완료입니다.</p></div><button type="button" onClick={orders.reload} className="rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm font-bold text-slate-900">새로고침</button></div>
