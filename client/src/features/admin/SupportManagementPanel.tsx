...
export function SupportManagementPanel() {
  ...
  const selectInquiry = (inquiry: AdminSupportInquiry) => { setSelectedId(inquiry.id); setReply(inquiry.adminReply ?? '') }
  ...
  const changeStatus = async (status: SupportInquiryStatus) => {
    ...
    try {
      ...
      notify(status === 'CLOSED' ? '민원을 처리 완료로 변경하고 고객에게 완료 알림을 발송했습니다.' : `민원을 ${STATUS_LABEL[status]} 상태로 변경했습니다.`)
  ...
  const saveReply = async () => {
    ...
    try {
      ...
      await Promise.resolve(inquiries.reload())
      notify('답변을 등록했습니다. 고객센터 알림으로 답변 발송이 완료되었습니다.')
