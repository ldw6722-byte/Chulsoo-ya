...
type AdminView = 'overview' | 'orders' | 'stores' | 'catalog' | 'finance' | 'support'
...
const NAVIGATION: Array<{ label: string; items: NavItem[] }> = [
  ...
  },
  {
    label: '고객 지원',
    items: [
      { label: '클레임 · 쿠폰', view: 'support', detail: '교환·반품·보상 쿠폰' },
    ],
  },
]
...
const VIEW_TITLE: Record<AdminView, [string, string]> = {
  ...
  support: ['클레임 · 쿠폰', '클레임 발생 시 정산을 보류하고 보상 쿠폰 정책을 관리합니다.'],
}
...
export function AdminOverviewPage() {
  ...
  const selectView = (view: AdminView) => { setActiveView(view); setSidebarOpen(false) }
