...
import { adminApi, adminUserApi } from '@/api/endpoints'
...
import type { AdminOverview, FeaturePermissionView, OrderStatus } from '@/types/api'
...
const isAdminView = (value: string | null): value is AdminView => ['overview', 'orders', 'stores', 'users', 'events', 'applications', 'catalog', 'finance', 'developmentPayments', 'coupons', 'subscriptions', 'support'].includes(value ?? '')
...
export function AdminOverviewPage() {
  ...
  const canAccess = (view: AdminView) => highestAdministrator || view === 'overview' || allowedCodes.has(VIEW_PERMISSION[view] ?? '')
  ...
  const [activeView, setActiveView] = useState<AdminView>(() => {
    ...
    return isAdminView(savedView) ? savedView : 'overview'
  })
  const [sidebarOpen, setSidebarOpen] = useState(false)
  const [query, setQuery] = useState('')
  useEffect(() => {
    if (!permissionViews.loading && !canAccess(activeView)) setActiveView('overview')
