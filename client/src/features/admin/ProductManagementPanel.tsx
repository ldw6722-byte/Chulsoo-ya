...
export function ProductManagementPanel() {
  ...
  const editor = <div className='rounded-2xl border border-brand-200 bg-brand-50 p-4'>
    ...
    <div className='mt-4 rounded-xl border border-rose-200 bg-rose-50/60 p-3'><div className='flex flex-wrap items-center justify-between gap-2'><div><h4 className='font-black text-slate-900'>철수야 셀렉트 행사</h4><p className='mt-1 text-xs text-slate-500'>기본 판매 가격을 행사 전 가격으로 사용하며, 할인율로 행사 적용가를 계산합니다.</p></div><label className='inline-flex items-center gap-2 text-sm font-bold text-slate-700'><input type='checkbox' checked={draft.selectPromotion} onChange={event => setDraft(current => ({ ...current, selectPromotion: event.target.checked }))} /> 행사 적용</label></div>{draft.selectPromotion ? <div className='mt-3 grid gap-3 md:grid-cols-3'><label className='text-sm font-bold text-slate-700'>할인율 (%)<input type='number' min='0' max='99' value={draft.discountRate} onChange={event => setDraft(current => ({ ...current, discountRate: event.target.value }))} className='mt-1 w-full rounded-xl border border-slate-300 bg-white px-3 py-2 font-normal' /></label><div className='rounded-xl border border-rose-200 bg-white px-3 py-2'><p className='text-xs font-bold text-slate-500'>행사 적용 판매가</p><p className='mt-1 text-xl font-black text-rose-600'>{promotionPrice(draft).toLocaleString()}원</p></div><label className='text-sm font-bold text-slate-700 md:col-span-3'>행사 카테고리<select value={draft.eventCampaignId} onChange={event => setDraft(current => ({ ...current, eventCampaignId: event.target.value }))} className='mt-1 h-11 w-full rounded-xl border border-slate-300 bg-white px-3 font-normal'><option value=''>행사 카테고리를 선택해 주세요.</option>{eventCampaigns.filter(item => item.active).map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label></div> : null}</div>
