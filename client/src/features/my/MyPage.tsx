...
import type { Cart, FeaturePermissionView, MemberProfile, OrderSummary, SellerDeactivationRequest } from '@/types/api'

const MENU_GROUPS = [
  { title: 'MY 주문', links: [['📦', '주문·배송·매칭 내역', '/orders'], ['↩️', '취소·환불 안내', '/orders'], ['🏪', '판매자 매칭 안내', '/catalog']] },
  { title: 'MY 혜택', links: [['🎫', '쿠폰·할인', '/catalog'], ['💰', '주문 혜택 안내', '/catalog']] },
  { title: 'MY 정보', links: [['👤', '회원 정보', '/my/profile'], ['🛒', '장바구니', '/cart'], ['📍', '배송지 관리', '/my/delivery-addresses'], ['💳', '결제수단', '/my/payment-methods'], ['⚙️', '계정 설정', '/my']] },
] as const
...
export function MyPage() {
  ...
  const permissions = useAsync<FeaturePermissionView[]>(() => role === 'ADMIN' ? userApi.myFeaturePermissions() : Promise.resolve([]), [identity?.userId, role])
  const isHighestAdministrator = user?.adminLevel === 'HIGHEST'
  const hasSellerFeature = (code: string) => isSeller || isHighestAdministrator || Boolean(permissions.data?.some(permission => permission.code === code && permission.enabled))
  const sellerWorkflowActive = profile.data?.sellerWorkflowActive ?? false
  const hasSellerWorkflow = isSeller || (sellerWorkflowActive && ['SELLER_STORE_OPERATIONS', 'SELLER_BID_AND_FULFILLMENT', 'SELLER_SUBSCRIPTION', 'SELLER_CLAIM_RESPONSE', 'SELLER_CATALOG'].some(hasSellerFeature))
  const canApplyForSeller = role === 'CONSUMER'
  const canApplyForAdministratorSeller = role === 'ADMIN' && !sellerWorkflowActive && (isHighestAdministrator || hasSellerFeature('CONSUMER_SELLER_APPLICATION'))
    ...
    <div className="mx-auto max-w-7xl px-4 py-7 md:py-10">
      ...
      <div className="mt-7 grid gap-6 lg:grid-cols-[245px_1fr]">
        <aside className="h-fit rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900">
          ...
          {canApplyForSeller && <section className="border-b border-slate-100 py-4 dark:border-slate-800"><h2 className="mb-2 px-2 text-xs font-black text-brand-600">SELLER PARTNER</h2><Link to="/seller/application" className="flex items-center gap-2 rounded-xl px-2 py-2.5 text-sm font-semibold text-slate-600 transition hover:bg-brand-50 hover:text-brand-700">🏪 판매자 등록 신청</Link></section>}{canApplyForAdministratorSeller && <section className="border-b border-slate-100 py-4 dark:border-slate-800"><h2 className="mb-2 px-2 text-xs font-black text-violet-600">ADMIN SELLER TEST</h2><Link to="/seller/application" className="flex items-center gap-2 rounded-xl px-2 py-2.5 text-sm font-semibold text-slate-600 transition hover:bg-violet-50 hover:text-violet-700">🧪 관리자 판매자 신청</Link></section>}{isSeller && <section className="border-b border-slate-100 py-4 dark:border-slate-800"><h2 className="mb-2 px-2 text-xs font-black text-rose-600">SELLER ACCOUNT</h2><Link to="/seller/deactivation" className="flex items-center gap-2 rounded-xl px-2 py-2.5 text-sm font-semibold text-rose-600 transition hover:bg-rose-50">📝 {deactivation.data?.status === 'PENDING' ? '판매자 등록 해지 신청 검토 중' : '판매자 등록 해지 신청'}</Link></section>}
          {hasSellerWorkflow && <section className="border-b border-slate-100 py-4 dark:border-slate-800"><h2 className="mb-2 px-2 text-xs font-black text-violet-600">SELLER WORKFLOW</h2><ul className="space-y-1"><li><Link to="/seller-guide#operations" className="flex items-center gap-2 rounded-xl px-2 py-2.5 text-sm font-semibold text-slate-600 transition hover:bg-violet-50 hover:text-violet-700">📘 판매자 안내 가이드</Link></li>{hasSellerFeature('SELLER_BID_AND_FULFILLMENT') && <><li><Link to="/seller/offers" className="flex items-center gap-2 rounded-xl px-2 py-2.5 text-sm font-semibold text-slate-600 transition hover:bg-violet-50 hover:text-violet-700">📥 응찰 내역</Link></li><li><Link to="/seller/orders" className="flex items-center gap-2 rounded-xl px-2 py-2.5 text-sm font-semibold text-slate-600 transition hover:bg-violet-50 hover:text-violet-700">📦 낙찰 후 주문 처리</Link></li></>}{hasSellerFeature('SELLER_CLAIM_RESPONSE') && <li><Link to="/seller/claims" className="flex items-center gap-2 rounded-xl px-2 py-2.5 text-sm font-semibold text-slate-600 transition hover:bg-violet-50 hover:text-violet-700">📣 클레임 처리</Link></li>}{hasSellerFeature('SELLER_STORE_OPERATIONS') && <li><Link to="/seller/settings" className="flex items-center gap-2 rounded-xl px-2 py-2.5 text-sm font-semibold text-slate-600 transition hover:bg-violet-50 hover:text-violet-700">⚙️ 판매자 운영 설정</Link></li>}</ul></section>}
