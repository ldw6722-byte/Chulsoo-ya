...
function ActiveSellerStatus({ email }: { email: string }) {
  return <section className="rounded-2xl border border-emerald-200 bg-emerald-50/60 p-6 shadow-sm dark:border-emerald-900/70 dark:bg-emerald-950/20"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-black tracking-wider text-emerald-700 dark:text-emerald-300">SELLER ACCOUNT ACTIVE</p><h1 className="mt-1 text-2xl font-black text-slate-900 dark:text-white">판매자 계정이 활성화되어 있습니다</h1><p className="mt-2 text-sm leading-6 text-slate-600 dark:text-slate-300">{email} 계정은 이미 판매자 기능을 사용할 수 있습니다. 같은 계정으로 판매점 신청을 다시 접수할 필요가 없습니다.</p></div><span className="rounded-full bg-emerald-100 px-3 py-1.5 text-sm font-black text-emerald-800 dark:bg-emerald-950 dark:text-emerald-200">승인 완료</span></div><div className="mt-6 rounded-xl bg-white/80 p-4 text-sm text-emerald-800 dark:bg-slate-900/80 dark:text-emerald-200">판매점 운영 설정·주문 응찰·구독 상태는 판매자 대시보드에서 확인할 수 있습니다.</div><Link to="/seller" className="guide-cta-primary mt-6 inline-flex">판매자 대시보드로 이동</Link></section>
}
