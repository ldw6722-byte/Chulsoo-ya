...
function nextActions(order: AssignedOrder): { label: string; next: OrderStatus }[] {
  if (order.status === 'PREPARING') return order.fulfillmentMethod === 'DELIVERY' ? [{ label: '배달 출발', next: 'DELIVERY_IN_PROGRESS' }] : [{ label: '픽업 준비 완료', next: 'PICKUP_READY' }]
  if (order.status === 'DELIVERY_IN_PROGRESS' || order.status === 'PICKUP_READY') return [{ label: '거래 완료 처리', next: 'COMPLETED' }]
  return []
}
