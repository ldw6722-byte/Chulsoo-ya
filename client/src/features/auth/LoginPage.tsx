...
export function LoginPage() {
  ...
  const submit = async (event: FormEvent) => {
    ...
    } catch (error) { setError(error instanceof AuthConfigurationError ? error.message : "Login failed. Please try again.") }
    finally { setLoading(false) }
  }
  return <AuthLayout>
    <h1 className="auth-heading">{"\uB85C\uADF8\uC778"}</h1>
    <p className="auth-subtitle">{"\uCCA0\uC218\uC57C\uC5D0 \uC624\uC2E0 \uAC83\uC744 \uD658\uC601\uD569\uB2C8\uB2E4."}</p>
    <SocialAuthButtons action="login" disabled={loading || !isSupabaseConfigured} onError={setError} nextPath={redirectPath} />
    ...
    <form className="auth-form" onSubmit={submit}>
      ...
      <div className="flex items-center justify-between text-sm"><label className="flex items-center gap-2 text-slate-600"><input type="checkbox" checked={remember} onChange={(event) => setRemember(event.target.checked)} />{"\uB85C\uADF8\uC778 \uC0C1\uD0DC \uC720\uC9C0"}</label><Link to="/support" className="auth-inline-link">{"\uBE44\uBC00\uBC88\uD638 \uCC3E\uAE30"}</Link></div>
      <button className="btn btn-primary btn-block" type="submit" disabled={loading || !isSupabaseConfigured}>{loading ? "Logging in..." : "\uB85C\uADF8\uC778"}</button>
    </form>
