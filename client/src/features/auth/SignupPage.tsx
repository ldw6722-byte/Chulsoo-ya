...
export function SignupPage() {
  ...
  const submit = async (event: FormEvent) => {
    event.preventDefault(); setError(null); setMessage(null)
        if (!agreed) { setError("이용약관 및 개인정보처리방침에 동의해 주세요."); return }
    ...
    try {
      ...
      if (data.session) { await refresh(); navigate(redirectPath, { replace: true }); return }
            setMessage("이메일 인증을 완료한 뒤 로그인해 주세요.")
    } catch (error) { setError(error instanceof AuthConfigurationError ? error.message : "회원가입에 실패했습니다. 다시 시도해 주세요.") }

    finally { setLoading(false) }
  }
    return <AuthLayout variant="signup">
    ...
    <form className="auth-form" onSubmit={submit}>
      ...
      <button className="btn btn-primary btn-block" type="submit" disabled={loading || !isSupabaseConfigured}>{loading ? "가입 중..." : "\uD68C\uC6D0\uAC00\uC785"}</button>
    </form>
