...
import { cartApi, orderApi, supportApi } from '@/api/endpoints'
  ...
  const submit = async () => {
    ...
    try {
      ...
      setTitle(''); setContent(''); setNotice('문의가 접수되었습니다. 답변이 등록되면 알림으로 안내합니다.')
            await onSubmitted()
      window.dispatchEvent(new Event('chulsooya:notifications-updated'))
